...
function checkChecklist() {
  ...
  if (checked){
    ...
    if (!Array.from(document.querySelector('#checklist_submit').classList).includes('hidden')) {
...
function handleMiscItemClick() {
  ...
  if (cup_type){
    itemName = 'Misc. $' + itemPrice.toFixed(2).toString() + ' Item (' + inputted_name +  'w/' + cup_type + (cup_type === 'Bottle' ? '' : 'cup');
  } else {
    itemName = 'Misc. $' + itemPrice.toFixed(2).toString() +  'Item (' + inputted_name + ')'
  }
  if(this.dataset.action === "remove") {
    if (localStorage[itemName]) {
      var o = localStorage.getItem(itemName)
